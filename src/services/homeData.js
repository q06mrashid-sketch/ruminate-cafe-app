...
export async function getToday() {
  try {
    ...
    if (key === 'fri') {
      ...
      const o = parseHHMM(cms?.[`hours.${key}.open`]), c = parseHHMM(cms?.[`hours.${key}.close`]);
...
// Legacy placeholders (kept to avoid import errors elsewhere)
...
export async function openInstagramUrl(url) {
  ...
  try { await Linking.openURL(url); } catch { openInstagramProfile(); }
}
...
export async function getLatestInstagramPost() {
  ...
  // Try Instagram web profile API first (works in RN; no browser CORS)
  try {
    ...
    // Fallback to a configurable feed endpoint if provided
    try {
