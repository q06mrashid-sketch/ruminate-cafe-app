...
function statusForIntervals(iv, now){ 
  if(!Array.isArray(iv)||!iv.length) return { openNow:false, until:'--:--', open:'--:--', close:'--:--' }; 
  const t=now.getHours()*60+now.getMinutes(); 
  for(const [a,b] of iv){ 
    if(t>=a && t<b) return { openNow:true, until:hhmm(b), open:hhmm(iv[0][0]), close:hhmm(iv[iv.length-1][1]) }; 
  } 
  const next=iv.find(([a])=>t<a); 
  if(next) return { openNow:false, until:hhmm(next[0]), open:hhmm(iv[0][0]), close:hhmm(iv[iv.length-1][1]) }; 
  return { openNow:false, until:'--:--', open:hhmm(iv[0][0]), close:hhmm(iv[iv.length-1][1]) }; 
}
function todayStatus(cms, now=new Date()){ 
  ...
  if(key==='fri'){ 
    const o1=parseHHMM(get('hours.fri.open1')), c1=parseHHMM(get('hours.fri.close1')), o2=parseHHMM(get('hours.fri.open2')), c2=parseHHMM(get('hours.fri.close2')); 
    const iv=[]; 
    if(o1!=null&&c1!=null&&c1>o1) iv.push([o1,c1]); 
    if(o2!=null&&c2!=null&&c2>o2) iv.push([o2,c2]); 
    return statusForIntervals(iv, now); 
  } else { 
    const o=parseHHMM(get(`hours.${key}.open`)), c=parseHHMM(get(`hours.${key}.close`)); 
    const iv=[]; 
    if(o!=null&&c!=null&&c>o) iv.push([o,c]); 
    return statusForIntervals(iv, now); 
  } 
}
...
export async function getWeeklyHours(){ 
  try{ 
    ...
    for(const d of days){ 
      if(d.k==='fri'){ 
        const o1=parseHHMM(cms?.['hours.fri.open1']), c1=parseHHMM(cms?.['hours.fri.close1']); 
        const o2=parseHHMM(cms?.['hours.fri.open2']), c2=parseHHMM(cms?.['hours.fri.close2']); 
        const seg1=(o1!=null&&c1!=null&&c1>o1)?`${hhmm(o1)}–${hhmm(c1)}`:null; 
        const seg2=(o2!=null&&c2!=null&&c2>o2)?`${hhmm(o2)}–${hhmm(c2)}`:null; 
        const text=seg1&&seg2?`${seg1} / ${seg2}`:(seg1||seg2||'Closed'); 
        out.push({ key:d.k, label:d.label, text }); 
      } else { 
        const o=parseHHMM(cms?.[`hours.${d.k}.open`]), c=parseHHMM(cms?.[`hours.${d.k}.close`]); 
        const text=(o!=null&&c!=null&&c>o)?`${hhmm(o)}–${hhmm(c)}`:'Closed'; 
        out.push({ key:d.k, label:d.label, text }); 
      } 
    } 
    return out; 
    ...
    return [
      {key:'mon',label:'Mon',text:'--:--'},
      {key:'tue',label:'Tue',text:'--:--'},
      {key:'wed',label:'Wed',text:'--:--'},
      {key:'thu',label:'Thu',text:'--:--'},
      {key:'fri',label:'Fri',text:'--:--'},
      {key:'sat',label:'Sat',text:'--:--'},
      {key:'sun',label:'Sun',text:'--:--'}
    ]; 
  } 
}
