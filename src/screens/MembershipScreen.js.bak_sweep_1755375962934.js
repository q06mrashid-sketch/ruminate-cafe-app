...
import { getMembershipSummary, signOut } from '../services/membership';
...
export default function MembershipScreen({ navigation }) {
  ...
  useEffect(() => { refresh(); }, []);
  useFocusEffect(React.useCallback(() => { let on = true; (async()=>{ if(on) await refresh(); })(); return () => { on = false; }; }, []));
  ...
  <View style={{ marginTop: 16 }}>
    <GlowingGlassButton
      ...
      onPress={async () => {
        ...
        try { setSummary({ signedIn:false, tier:'free', status:'none', next_billing_at:null }); } catch (e) {}
        try { navigation && navigation.reset({ index: 0, routes: [{ name: 'Home' }] }); } catch (e) {}
      }}
    />
  </View>
) : null}
