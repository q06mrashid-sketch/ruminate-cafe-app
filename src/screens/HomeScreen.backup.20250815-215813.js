...
import HomeSignedInPanel from '../components/home/HomeSignedInPanel';
import HomeSignedOutPanel from '../components/home/HomeSignedOutPanel';
...
export default function HomeScreen({ navigation }) {
  const [member, setMember] = useState({ signedIn: false, tier: 'free', status: 'none', next_billing_at: null });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (s1 || s2) {
          try { setToday(prev => ({ ...prev, specials: [s1, s2].filter(Boolean) })); } catch {}
        }
        if (cms['rumi quote']) {
          try { setRumiQuote(cms['rumi quote']); } catch {}
        }
      } catch {}
    })();
    return () => { mounted = false; };
            ...
            <>
              <Pressable onPress={() => navigation.navigate('Community')} style={{ marginTop: 16 }}>
                ...
                <Text style={styles.muted}>
                  £{((fund?.total_cents || 0) / 100).toFixed(2)}{(fund?.goal_cents || 0) ? ` / £${((fund?.goal_cents || 0) / 100).toFixed(2)}` : ''}
                </Text>
              </Pressable>
